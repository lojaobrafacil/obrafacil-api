class ApiPartner::WelcomesController < ApplicationController
  def web
    ...
    @welcomes = {
      highlights: (Highlight.where("(expires_at is null or expires_at > '#{Time.now}') and status = 1").order("position DESC NULLS LAST, expires_at").limit(3).map { |u| ActiveModelSerializers::Adapter.configured_adapter.new(ApiPartner::HighlightSerializer.new(u)).serializable_hash } rescue nil),
