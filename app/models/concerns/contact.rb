module Contact
  def contacts
    e = self.emails.select("emails.*, email_types.name as email_type_name").joins(:email_type).to_a
    p = self.phones.select("phones.*, phone_types.name as phone_type_name").joins(:phone_type).to_a
    a = self.addresses.select("addresses.*, address_types.name as address_type_name, cities.name as city_name,
